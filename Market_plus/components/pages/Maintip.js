...
import { Destroy, GetN, GetUserInfo, Store, Update } from "../Send/Send";
...
const Maintip = () => {
    ...
    const createData = async () => {
        var data = !onid ? await Store('POST','create_maintip',
            await this.setColum()
        ) : await Update('PUT',`update_maintip/${onid}`,
            await this.setColum()
        )
    ...
    const delete_tavar = async () => {
        var data = await Destroy('DELETE',`delete_maintip/${ondeletid}`);
        data.status === 201 ? await clerdel() : null;
    }

    const getList = async (correntPage = 0) => {
        setData(await GetUserInfo('getlist_maintip', correntPage, search || null));
    }
