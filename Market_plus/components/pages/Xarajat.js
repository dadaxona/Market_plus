...
const Xarajat = () => {
    ...
    const createTavar = async () => {
        var data = !onid ? await Store('POST','create_rasxod',
            await this.setColum()
        ) : await Update('PUT',`update_rasxod/${onid}`,
            await this.setColum()
        )
    ...
    const getList = async (correntPage = 0) => {
        onID('');
        setData(await GetN(await USER_ID(), 'getlist_rasxod', correntPage, search || null));
    }
    ...
    const renderItem = () => {
        return (
            ...
            return <View
                ...
                key={key}
                >
                <Text className="text-lg">{items.why}</Text>
  ...
  return (
    <View className="m-1" style={{
        ...
        <View style={{
            ...
            <View className="flex-row">
                <Text className="border border-slate-400 rounded-md px-3 py-1" style={{width: 50}} onPress={() => TuggleModal(true)}>
                    <Entypo
                        name="add-to-list" 
                        size={25} 
                        color="green"
